...
class _AdminSearchScreenState extends State<AdminSearchScreen> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      final kitabResponse = await SupabaseService.from(
        'kitab',
    ...
    setState(() {
      ...
      if (query.isEmpty) {
        ...
        _searchResults = _allKitabList.where((k) {
          ...
          final authorMatches =
              k.author?.toLowerCase().contains(searchLower) ?? false;
    ...
    if (_error != null) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(HugeIcons.strokeRoundedAlert02, size: 64.0, color: Colors.red),
