-- YouTube Auto Sync Scheduler Migration
-- This creates a periodic sync system for YouTube playlists

-- Create YouTube sync logs table to track sync operations
...
CREATE INDEX IF NOT EXISTS idx_youtube_sync_logs_kitab_id ON youtube_sync_logs(video_kitab_id);
CREATE INDEX IF NOT EXISTS idx_youtube_sync_logs_status ON youtube_sync_logs(status);
CREATE INDEX IF NOT EXISTS idx_youtube_sync_logs_created_at ON youtube_sync_logs(created_at);

-- Create YouTube sync settings table for global configuration
...
COMMENT ON TABLE youtube_sync_logs IS 'Tracks YouTube playlist sync operations and results';
COMMENT ON TABLE youtube_sync_settings IS 'Global configuration for YouTube sync system';
COMMENT ON FUNCTION log_youtube_sync IS 'Records sync operation results for tracking and debugging';
COMMENT ON FUNCTION get_playlists_for_sync IS 'Returns playlists that need to be synced based on schedule';
COMMENT ON FUNCTION update_kitab_after_sync IS 'Updates video kitab statistics after successful sync';